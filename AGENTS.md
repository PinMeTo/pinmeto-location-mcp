...
- The only release/build deliverable is the `.mcpb` Desktop Extension (`npx @anthropic-ai/mcpb pack`, running `build/index.js`), attached to GitHub releases by `release:draft`. There is no single-file bundle: the `npm run bundle` → `dist/index.mjs` target existed only to be vendored by the `PinMeTo/claude-plugins` marketplace, which became skill-only in 2026-08 and no longer consumes it, so it was retired.

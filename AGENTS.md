...
- **Don't override the SDK's `initialize` or `server/discover` handlers.** Capabilities are derived from what `createMcpServer()` registers. A previous hand-written initialize override advertised a `resources` capability this server never implemented. `serveStdio(() => createMcpServer())` selects the protocol era and uses the SDK handlers for both openings.
- **Client identity is resolved per request** in `PinMeToMcpServer._userAgent(context)`, never written to `axios.defaults`. Modern requests read `clientInfo` from `ctx.mcpReq.envelope[CLIENT_INFO_META_KEY]`; legacy requests fall back to the identity captured by `initialize`. Every tool path that calls PinMeTo must pass its handler context through the request helpers.
- **Server identity is era-specific on the wire.** Legacy clients receive `serverInfo` in the `initialize` result. Modern clients receive it in `_meta['io.modelcontextprotocol/serverInfo']` on every response. The SDK stamps the same `Implementation` object in both cases, including `description` and `websiteUrl`; do not add custom handshake handlers to move or gate those fields.
- **The MCP SDK is v2 (`@modelcontextprotocol/server`) with Zod 4. Do not go back to `@modelcontextprotocol/sdk` 1.x.** The v1 line ended at 1.30.0 and emits JSON Schema draft-07 on every tool schema with no opt-out (typescript-sdk#2084); Claude Desktop rejects that dialect on `outputSchema`, which broke every tool call for customers (PinMeTo/issues#6064). v2 emits 2020-12, and `tests/mcp_server.test.ts` pins that dialect. Tool handler content blocks need `type: 'text' as const`; v2's result types no longer accept a widened string.

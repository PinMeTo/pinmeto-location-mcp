...
- The `.mcpb` is packed from the working directory, not from git. `.mcpbignore` uses gitignore semantics (the packer uses the `ignore` library and does not read `.gitignore`): keep every rule anchored with a leading `/`, because a bare `dist` also strips `node_modules/*/dist`. `npm run verify:mcpb` (run by `release:draft` and `pack:test`) unpacks the bundle, rejects unsafe paths and unexpected top-level entries, and starts the packed server. The first v4.1.0 asset shipped without this and failed to install.
